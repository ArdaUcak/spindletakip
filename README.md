...
- Tarayıcıdan `http://<sunucu-ip>:<port>` (veya gerekirse `/launch` alt yolu) adresine giderek Spindle ve Yedek kayıtlarını ekleyip/güncelleyip/silebilir, Referans ID ile arayabilir ve doğrudan CSV dışa aktarımı tetikleyebilirsiniz. Tüm işlemler aynı CSV dosyalarını (`spindle_data.csv`, `yedek_data.csv`, `takip_export.csv`) kullanır ve **GG-AA-YYYY** tarih damgası otomatik uygulanır. Yanıtlar `Cache-Control: no-store` başlığıyla sunulduğu için eski lansman sayfaları tarayıcı önbelleğinden gelmez; sayfayı yenileyerek en güncel web arayüzünü görürsünüz.【F:claudsys.js†L24-L182】【F:claudsys.js†L185-L324】【F:claudsys.js†L326-L371】
