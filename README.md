...
- `node claudsys.js` komutu, 0.0.0.0 üzerinde **STS-SpindleTakipSistemi** web arayüzünü bağımsız olarak sunar (varsayılan port **8000**, `CLAUDSYS_PORT` ile değiştirilebilir; port doluysa otomatik olarak bir sonraki porta geçer ve konsola bildirir). Masaüstü uygulamasına dokunmaz; tamamen webden çalışır.【F:claudsys.js†L1-L22】【F:claudsys.js†L334-L367】
- Tarayıcıda `http://<sunucu-ip>:<port>` (veya `/launch`) adresine gidince giriş adımı olmadan SPA doğrudan açılır. Sekmeli Spindle/Yedek listeleri, modal ekle/düzenle diyalogları ve Referans ID ile arama çubukları web üzerinden çalışır. CSV dışa aktarımı, satır düzenleme/silme ve otomatik **GG-AA-YYYY** tarih damgası web tarafında aynı CSV şemasına kaydedilir. Yanıtlar `Cache-Control: no-store` ile sunulur, böylece her yenilemede güncel arayüz yüklenir.【F:claudsys.js†L24-L329】【F:claudsys.js†L331-L367】
- Verilerin nereye yazılacağı `CLAUDSYS_DATA_DIR` ortam değişkeniyle değiştirilebilir; aksi hâlde script’in bulunduğu klasöre (`spindle_data.csv`, `yedek_data.csv`, `takip_export.csv`) yazar. Bu ayar, `main.py` dosyasını veya masaüstü verilerini etkilemeden web arayüzünü bağımsız kılar.【F:claudsys.js†L10-L32】【F:claudsys.js†L47-L72】
- Masaüstü GUI (`main.py`) değişmez: Claudsys web arayüzü `main.py` dosyasını çalıştırmaz ya da düzenlemez; masaüstü sürümündeki güncellemeler yalnızca isteğe bağlı olarak yapılmıştır ve mevcut haliyle bırakılır.
